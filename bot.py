...
async def handle_application(message: types.Message):
    ...
    for photo in message.photo or []:
        content.append(f"[Фото: {photo.file_id}]")
    if message.video:
        content.append(f"[Видео: {message.video.file_id}]")

    await bot.send_message(GROUP_ID, f"{' '.join(content)}\nОтправил заявку: @{username}\nДолжность: {role}\nКак заполнена анкета: {message.text or 'Файлы'}", reply_markup=markup)
