...
dp = Dispatcher(bot)  # Передаем бота в диспетчер
...
@dp.callback_query_handler(lambda callback: callback.data.startswith("approve_") or callback.data.startswith("reject_"))
...
@dp.message_handler(lambda message: message.text.startswith("+выплата"))
...
async def main():
    await init_db()
    await dp.start_polling()  # Теперь без передачи бота
