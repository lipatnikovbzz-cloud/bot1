...
async def start_command(message: types.Message):
    ...
    markup.add(KeyboardButton("Курьер"), KeyboardButton("Дроповод"))
    markup.add(KeyboardButton("Доп. Вакансии"), KeyboardButton("Реф. Система"))
    await message.answer("Какую именно должность Вы хотите занять?", reply_markup=markup)

@dp.message_handler(lambda message: message.text in ["Курьер", "Дроповод", "Доп. Вакансии"])
...
async def zp_command(message: types.Message):
    ...
    try:
        ...
        _, username, amount, payment_link = parts + [""] if len(parts) == 3 else parts
        username = username.replace("@", "")
        amount = int(amount)
        if not re.match(r"^t\.me/send\?start=", payment_link):
            await message.answer("Ошибка: ссылка должна начинаться с t.me/send?start=")
            return
    except ValueError:
        await message.answer("Формат: /zp @username сумма [ссылка]")
        return
...
@dp.message_handler(commands=['zpall'])
